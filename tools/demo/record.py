...
titles=['카메라 시야각 조정','지구 선택 · 한 물체 이동','선택한 지구만 회전','광원 하나 이동','카메라 회전']
controls=['1 → E → 숫자패드 -','지구 클릭 → 3 → W → 숫자패드 6 / 4','3 → Q → 숫자패드 8 / 2','2 → W → 숫자패드 6 / 4','1 → Q → 숫자패드 4 / 6']
...
try:
 for f in range(600):
  ...
  draw.text((932,20),f'{f//120+1:02} / 05',font=small,fill='#bbc6d2')
  draw.text((24,523),controls[f//120],font=small,fill='#d6e1ee')
  draw.rectangle((0,557,int(1050*(f+1)/600),559),fill='#5bbbc6')
  if 120<=f<140:
   x,y=437,225+62
   draw.ellipse((x-10,y-10,x+10,y+10),outline='#ffd166',width=2)
  if f in (90,160,298,414,514): frame.save(output/f'chapter-{f//120+1}.png')
