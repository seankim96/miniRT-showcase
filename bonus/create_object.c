...
/*   Updated: 2025/08/18 10:51:04 by yeonjuki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture	*get_texture(char *token, t_object_type object_type)
{
	char		*tex_name;
	t_texture	*tex;
	t_color		color;
	...
	if (ft_strncmp(token, "tex:", 4) == 0 && ft_strlen(token) > 4)
	{
		...
		if (ft_strncmp(tex_name, "checker", 8) == 0)
			...
			(t_vector){0, 0, 0}, (t_vector){255, 255, 255}, \
			get_checker_scale(object_type));
	...
	double radius)
{
	...
	tex = get_texture(tex_token, SPHERE);
	bump = NULL;
	if (!tex)
		return (NULL);
	...
	t_vector normal)
{
	...
	tex = get_texture(tex_token, PLANE);
	if (!tex)
		return (NULL);
	...
	double dval[2])
{
	...
	tex = get_texture(tex_token, CYLINDER);
	bump = NULL;
	if (!tex)
		return (NULL);
	if (get_bumpmap(tex_token, &bump) < 0)
	...
	double dval[2])
{
	...
	tex = get_texture(tex_token, HYPERBOLOID);
	bump = NULL;
	if (!tex)
		return (NULL);
	if (get_bumpmap(tex_token, &bump) < 0)
	...
	hy = make_wrapper(center, normal, (t_vector){dval[0], dval[1], dval[0]});
	if (!hy)
	{
		ft_putendl_fd("Error: Failed to create hyperboloid.", 2);
		tex->f.free(tex);
		bump->f.free(bump);
		return (NULL);
	}
	set_wrapper(hy, get_hyperboloid(), tex, bump);
	return (hy);
}
